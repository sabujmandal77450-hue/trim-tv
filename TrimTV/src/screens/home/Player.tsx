...
const Player = ({route}: Props): React.JSX.Element => {
  ...
  // ── Android TV / D-pad remote control ──────────────────────────────────
  useEffect(() => {
    if (!Platform.isTV) {
      return;
    }
    const tvHandler = new TVEventHandler();
    tvHandler.enable(null, (_cmp: any, event: any) => {
      const type: string = event?.eventType;
      switch (type) {
        case 'select':
          // Centre/OK button → toggle play-pause
        ...
        case 'mediaRewind':
          ...
          if (playerRef?.current) {
            playerRef.current.seek(Math.max(0, progress.currentTime - 10));
          }
          break;
        case 'right':
        case 'fastForward':
        case 'mediaFastForward':
          ...
          if (playerRef?.current) {
            playerRef.current.seek(
              Math.min(progress.duration, progress.currentTime + 10),
            );
          }
          break;
        case 'playPause':
