...
const main = async () => {
    const prettyQuickResult = await prettyQuick(process.cwd(), {
        ...
        onFoundSinceRevision: (scm, revision) => {
            console.log(`🔍  Finding changed files since ${picocolors.bold(scm)} revision ${picocolors.bold(revision)}.`);
        },
        onFoundChangedFiles: changedFiles => {
            console.log(`🎯  Found ${picocolors.bold(changedFiles.length)} changed ${changedFiles.length === 1 ? 'file' : 'files'}.`);
        },
